"""Pack-generation agent: system prompt, corpus grounding, and assembly.
...
(Layer 3, re-validation at save time, lives in ``PackRepository.create``.)

:func:`build_generation_agent` assembles the full agent — prompt, tool, and
validator — with NO bound model and no configuration, so this module is
importable and runnable with nothing but a model and a
:class:`GenerationDeps`. The production wiring (config gating, OpenRouter model
resolution, run logging, persistence) lives in
:mod:`habagou.services.pack_generation`; evaluation harnesses call the factory
directly with their own model and corpus (see ``docs/evals.md``).
"""
...
class CorpusReader(Protocol):
    ...
    ``habagou.repositories.characters.CharacterRepository`` satisfies it
    structurally; unit tests and eval harnesses supply a lightweight stub so no
    database is needed.
    """
...
class GenerationDeps:
    ...
    against a stub. The service wires a real ``CharacterRepository`` here.
    """
...
def build_generation_agent() -> Agent[GenerationDeps, PackDraft]:
    """Assemble the pack-generation agent with tool + validator wired.

    The agent is built WITHOUT a bound model so it can be imported, unit
    tested, and evaluated with no configuration and no network: callers supply
    the model at run time via ``agent.run(..., model=...)``, and tests inject a
    ``TestModel`` / ``FunctionModel`` the same way or via ``agent.override``.
    """
    ...
    async def _corpus_membership(ctx: RunContext[GenerationDeps]) -> str:
        """Append the full traceable corpus to the system prompt (the
        corpus-in-prompt change, "Move A").

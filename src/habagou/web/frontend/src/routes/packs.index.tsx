...
// The library entry point replaced the AI create card on the bench: curated
// packs are the primary path, so this card is always visible (no gating).
function BrowseLibraryCard() {
  return (
    <Link
      aria-label="Browse the library — enable curated packs from the library"
      className="mt-4 grid grid-cols-[3.5rem_1fr_auto] items-center gap-4 rounded-lg border border-jade/40 bg-jade/[0.04] p-4 text-left transition-colors hover:border-jade/60 hover:bg-jade/[0.08]"
      to="/packs/library"
    >
      <span
        aria-hidden="true"
      ...
      <span className="min-w-0">
        <span className="block text-base font-semibold leading-6">Browse the library</span>
        <span className="mt-0.5 block text-sm leading-5 text-mist">
          Enable curated packs from the library.
        </span>
      </span>

...
test("[WF-AUTH-SIGN-IN] signs in through Keycloak", async ({ page }) => {
  ...
  // The learner's identity now lives behind the account avatar.
  await expect(page.getByRole("button", { name: "Account: Dev User" })).toBeVisible();
});

test("[WF-AUTH-SIGN-OUT] signs out and returns to login", async ({ page }) => {
  await signIn(page);

  // Sign out is in the account menu, opened from the avatar.
  await page.getByRole("button", { name: "Account: Dev User" }).click();
